<template>
  <v-navigation-drawer v-model="drawer" color="secondary" :expand-on-hover="expandOnHover" :mini-variant="miniVariant" :permanent="permanent" dark app clipped>
    <v-list nav dense class="py-3">
      <v-list-item two-line :class="miniVariant && 'px-0'">
        ...
        <v-list-item-content>
          ...
          <v-list-item-subtitle>
            {{
                this.getAuthUser.name.replace(/\w\S*/g, w =>
      ...
      <div v-for="item in menuItems" :key="item.text">
        ...
        <template v-if="item.to.length > 0">
          <drawer-link :item="item" />
        </template>
        <template v-else>
          <drawer-sub-menu :item="item" />
        </template>
      </div>
    </v-list>
  </v-navigation-drawer>
...
<script>
import menu from "../../data/drawerItems";
...
export default {
	...
	computed: {
		...mapGetters(["drawerOn", "getDrawerItems", "getAuthUser"])
	},
	watch: {
		drawerOn: function(val) {
			this.drawer = val;
		}
	},
	mounted: function() {
    if(this.getAuthUser.role_id){
      this.menuItems = menu.menu.filter(item => {
        if (item.role.length == 0) return true;

        return item.role.includes(this.getAuthUser.role_id);
      })

      if (this.$route.meta.role.length > 0 && !this.$route.meta.role.includes(this.getAuthUser.role_id)){
