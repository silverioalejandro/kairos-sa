...
class ClickerClickIdExport implements FromCollection, WithMapping, WithHeadings
{
    use Exportable;
    ...
    public function collection()
    {
        ...
        $data = $this->model->with([])
            ->selectRaw("
                ...
                CASE 
                    WHEN ocx_click_id IS NULL OR ocx_click_id = '' OR ocx_click_id = 'undefined' THEN 'Unassigned'
                    ELSE ocx_click_id 
                END as ocx_click_id, 
                site, 
                count(1) total_click, 
                sum(cpc) as revenue, 
                (sum(cpc) / count(1)) average
            ")
            ->groupBy(DB::raw('DATE(created_at)'), DB::raw("
                CASE 
                    WHEN ocx_click_id IS NULL OR ocx_click_id = '' OR ocx_click_id = 'undefined' THEN 'Unassigned'
                    ELSE ocx_click_id 
                END
