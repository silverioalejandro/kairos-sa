...
class ClickerExportRaw implements FromCollection, WithMapping, WithHeadings
{
    use Exportable;
    ...
    */
    public function collection()
    {
        ...
        $data = $this->model->with([])
            ->selectRaw('min(id) id, DATE(created_at) as created_at, campaign, site, count(1) total_click, sum(cpc) as revenue, (sum(cpc) /count(1)) average, s as s, s2 as s2, s3 as s3, s4 as s4, s5 as s5')->groupBy(DB::raw('DATE(created_at)'), 'campaign', 'site', 's', 's2', 's3', 's4', 's5');
